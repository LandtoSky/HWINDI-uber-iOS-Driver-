...
@synthesize mapView_, coordinate;
...
- (void)viewDidLoad
{
    ...
    // Creates a marker in the center of the map.
    markerPassenger = [[GMSMarker alloc] init];
    ...
    flagForNav=(int)[[pref objectForKey:PREF_NAV] integerValue];
    if (flagForNav==1)
    {
...
-(void) showRouteFrom:(CLLocationCoordinate2D)f to:(CLLocationCoordinate2D)t
{
    ...
    if (strDesti_Latitude && strDesti_Longitude)
    {
        ...
        marker.icon = [UIImage imageNamed:@"marker_destination"];
        marker.map = mapView_;
...
- (MKAnnotationView *) mapView:(MKMapView *)mapView viewForAnnotation:(id <MKAnnotation>) annotation
{
    ...
    if (temp.yTag==1001)
    {
        annot.image=[UIImage imageNamed:@"marker_source"];
    }
    if (temp.yTag==1000)
    {
...
-(void)locationUpdated:(NSNotification*) notification
{
    ...
    CLLocation *newLocation = [[CLLocation alloc] initWithLatitude:[struser_lati doubleValue] longitude:[struser_longi doubleValue]];
    ...
    CLLocationDistance distance = [walkOldLocation distanceFromLocation:newLocation];
    if (distance>=20)
    {
        walkOldLocation=[walkOldLocation initWithLatitude:newLocation.coordinate.latitude longitude:newLocation.coordinate.longitude];
        [self updateWalkLocation];
